...
{Ações: Da acesso aos métodos privados de CRUD para o Controller}
...
{Alterar: Primeira faz a busca pra se certificar que os Atributos são os que
          estão persistidos e após isso coloca o Status para Alteração}
...
{Apagar: Primeiro faz uma busca para se certificar que existe um Log valido para
         exclusão e após isso executa o DELETE}
function TLog.Apagar: Boolean;
begin
  Result := False;

  {Verifica se não está Incluindo ou Alterando}
  if Status <> tstNavegando then
    raise Exception.Create('Log atual está em modo de inserção ou alteração!');
...
{ConsistenciaDados: Analisa as regras de negócio antes de algumas ações}
function TLog.ConsistenciaDados: Boolean;
begin
  Result := False;

  if Self.Codigo <= 0 then
    raise Exception.Create('Verifique o código do Download!');
...
begin
  {Cria a conexão com BD e já conecta em uma Query}
...
{Gravar: Método para persistência dos dados da Classe, o metodo ele verifica
